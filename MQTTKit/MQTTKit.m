...
// dictionary of mid -> completion handlers for messages published with a QoS of
// 1 or 2
...
static void on_message(struct mosquitto *mosq, void *obj,
  ...
  // The GCD autorelease pool isn't guaranteed to clean this up in any amount of
  // time.
  // Source:
  ...
  @autoreleasepool {
    ...
    NSData *payload =
        [NSData dataWithBytes:mosq_msg->payload length:mosq_msg->payloadlen];
...
static void on_subscribe(struct mosquitto *mosq, void *obj, int message_id,
                         int qos_count, const int *granted_qos) {
  MQTTClient *client = (__bridge MQTTClient *)obj;
  NSNumber *mid = [NSNumber numberWithInt:message_id];
  MQTTSubscriptionCompletionHandler handler =
      [client.subscriptionHandlers objectForKey:mid];
  if (handler) {
...
static void on_unsubscribe(struct mosquitto *mosq, void *obj, int message_id) {
  ...
  void (^completionHandler)(void) =
      [client.unsubscriptionHandlers objectForKey:mid];
...
- (instancetype)initWithClientId:(NSString *)clientId
  ...
  if ((self = [super init])) {
    ...
    mosq =
        mosquitto_new(cstrClientId, self.cleanSession, (__bridge void *)(self));
...
- (void)connectWithCompletionHandler:
  ...
  mosquitto_reconnect_delay_set(mosq, self.reconnectDelay,
                                self.reconnectDelayMax,
                                self.reconnectExponentialBackoff);
...
- (void)setWillData:(NSData *)payload
  ...
  mosquitto_will_set(mosq, cstrTopic, (int)payload.length, payload.bytes,
                     willQos, retain);
}
...
- (void)publishData:(NSData *)payload
  ...
  if (qos == 0 && completionHandler) {
    [self.publishHandlers setObject:completionHandler
                             forKey:[NSNumber numberWithInt:0]];
  }
  int mid;
  mosquitto_publish(mosq, &mid, cstrTopic, (int)payload.length, payload.bytes,
                    qos, retain);
  if (completionHandler) {
    if (qos == 0) {
      ...
      [self.publishHandlers setObject:completionHandler
                               forKey:[NSNumber numberWithInt:mid]];
    }
  }
}
...
completionHandler:(MQTTSubscriptionCompletionHandler)completionHandler {
  ...
  if (completionHandler) {
    [self.subscriptionHandlers setObject:[completionHandler copy]
                                  forKey:[NSNumber numberWithInteger:mid]];
  }
}
...
- (void)unsubscribe:(NSString *)topic
  ...
  if (completionHandler) {
    [self.unsubscriptionHandlers setObject:[completionHandler copy]
                                    forKey:[NSNumber numberWithInteger:mid]];
  }
}
